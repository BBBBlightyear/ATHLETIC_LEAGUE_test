...
public class Contest_dao extends BaseDao<Competition>{
    //--------------------------------------查询队伍表-----------------------------------------------//
    public int signUp(String CID, String C_name, String C_level, String C_class) {
        try {
            ...
            List<Competition> ct = seek(CID,null,null,null);
            if (ct!=null) {
    ...
    //--------------------------------------查询比赛-----------------------------------------------//
    public List<Competition> seek(String CID, String C_name, String C_level, String C_class) {
        try {
            String str ;
            str="SELECT * FROM contest_info Where (CID=? or ?='null') and （C_name=? or ?='null'） and （C_level=? or ?='null'） and （C_class = ? or ?='null'）";
            List<Competition> con= super.queryMulti(str.toString(), Competition.class,CID ,CID ,C_name,C_name, C_level,C_level, C_class, C_class );
            if (con==null) {
